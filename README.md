...
I'll occasionally refer to study 1, study 2, study 3, and study 4. Study 1 is our MDMA sample, 2 is psilocybin, 3 is LSD (humans), and 4 is LSD in mice.

If you want to find the code behind specific figures, I would use cntrl-f/apple-f for "2A", "3C", etc. within this README.md. It should point you to the script that actually generates the figure from the processed data.
...
### 1A. fMRI Preprocessing
  ...
  Next, this script combines the averaged left and right hemisphere TSNR maps. The bottom 15 percent of voxels (lowest TSNR) are masked out for all subsequent analyses. The same mask is applied to that dataset for equivalence: [TSNR_mask_3_avSNR-mw_to_label](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/TSNR_mask_3_avSNR-mw_to_label.m)

  Now all we need to do is downsample the mask to appropriate resolution for optical flow, fsaverage4. Here's that [script](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/DS_surf_TSNR.sh). [Connectome Workbench](https://www.humanconnectome.org/software/connectome-workbench) (wb_command) makes this relatively simple. The connectome workbench GUI was used to visualize the resultant TSNR mask for *Figure S1*.

  That's it for SNR. Next is the [xcpd](https://xcp-d.readthedocs.io/en/latest/) sbatching script (useful for head-motion-prone scan protocols, runs on fmriprep outputs): [sbatch_xcpd.sh](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/sbatch_xcpd.sh)

  The group behind this [paper](https://www.nature.com/articles/s41586-024-07624-5) was kind enough to send their data over already processed. For full transparency, the code used to download the images and their associated files are linked below:
  ...
  *Download psilocybin data: neuroimages* - [DL_psilo.sh](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/DL_psilo.sh)
  ...
  Please also see their more recent publication explicitly detailing this unique dataset as a [public resource](https://www.nature.com/articles/s41597-025-05189-0).

  Study 3 was also generously shared, originally detailed in this [study](https://www.sciencedirect.com/science/article/pii/S0924977X16300165). These data were processed as detailed in this [paper](https://www.nature.com/articles/s41586-024-07624-5).

  Now we'll downsample both datasets to optical flow resolution. Optical flow is a bit computationally intensive (prohibitively intensive at the moment for certain null modeling approaches), so we'll downsample these to fsaverage4 resolution rather than the fsaverage5 used for NMF. It uses the same basic code as downsampling to fsaverage5, but with fs4 as the target surface. Here is [DS_surf_ts_mdma.sh](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/DS_surf_ts_mdma.sh) for study 1 data. For study 2 (psilocyin data), we'll first need to run [RS_mask_psil](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/RS_mask_psil.m), which pulls just the resting state images out of the downloaded psilocybin data. After doing so, we can proceed with the equivalent downsampling scrip [DS_surf_ts_psil.sh](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/DS_surf_ts_psil.sh) for study 2 data. Study 3 was downsampled with this [script](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/DS_surf_ts_lsd.sh).

  OK, now we have all our processed data. To make it suitable for NMF and optical flow, we are going to motion-mask it. This goes a step further than normal temporal censoring: we are only interested in sequences of neuroimages uninterrupted by head motion. So rather than simply dropping high-motion frames, we'll only retain sequences of at least 8 TRs that are uninterrupted-by-motion (6 TRs for studies 2 and 3, for which TRs are longer units of real time). We'll save out the indices of these sequences for optical flow, as we're only interested in activity movement within clean segments (i.e., if frames 9 and 10 have high motion and we retain frames 1-8 and 11-20, we don't want to try and estimate activity movement between 8 and 11, just within the clean segments.) The script that takes care of this is [MotMask.m](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/MotMask.m). It also converts files to .mgh internally, which are easier formats for matlab to work with. As will be the case for many scripts below, an [equivalent script](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/MotMask_psil.m) exists for the psilocybin study and for the [human LSD study](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/MotMask_lsd.m). In general, the only differences are the input filepath and output file path. For this script in particular, some additional lines of code exist. As we get further in the pipeline, the file derivatives will be more equivalent, requiring fewer differences between MDMA study and Psilocybin Study scripts. A visual depiction of this motion mask can be found in *Figure S2*, which was generated using [this script](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Lobeplot.py)
...
*NMF steps*
  Alright, now that we have data in fsaverage5, we'll run it through the NMF pipeline. Basically the steps are to 1) prepare the data for internal NMF processing, 2) create a few candidate solutons using different combinations of brain scans 3) determine the optimal solution of the candidate solutions 4) convert the solution from a matrix to a proper brainmap for further use. Note that both human and mouse NMF utilized extra scans to for greater data volume and reliability. Also note you'll need the helper scripts that come with this code. This approach was developed by Hongming Li, who has made all analytic code available in this [repository](https://github.com/hmlicas/Collaborative_Brain_Decomposition). Slight adaptations were made to extend functionality to dorsal surface images in mice (1D).
  ...
  NMF script 4: this script just takes the derived solution and converts it to connectome workbench style file formats (giftis). Simple as. Here's the link to [Step_4_Extract_Group_Atlas.m](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/NMF/Step_4_Extract_Group_Atlas.m). *This is how you generate the files for figure S2, which are visualized in the connectome workbench GUI*
...
### 1D. DMN Derivation - mice
  ...
  NMF script 3.5: This one didn't get it's own arabic numeral because it's just a plotting script. Nice to triple check that everything is working as intended. Script can be found [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/NMF/Step_3andAHalf_Viz_Consensus_flat.m). *Panels for Figure S4 generated within.*
...
### 1D. DMN validation
  ...
  Ok, these are just some simple checks to make sure our derived DMN maps onto previous accounts of the DMN. Specifically, we'll use the spin test to see if our DMN is more aligned with previously published maps than 10,000 spun nulls. The [spatial permutation approach](https://github.com/spin-test/spin-test) allows us to better-account for autocorrelation in brain maps. There's a subfolder in the scripts directory for 1D, titled [DMN_validate](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/DMN_validate/). These scripts were used to generate *Figure S5*.

  We'll start with the ROIs from Goldstein-Piekarski et al., [Biological Psychiatry 2021](https://www.biologicalpsychiatryjournal.com/article/S0006-3223(21)01437-2/fulltext). More specifically, these ROIs are available at this [lab's github page](https://github.com/WilliamsPanLab/2021-masks). 
...
That's it for step 1! As noted above, data formats were disimilar upon receipt, but are gradually standardized into similar filenames, extensions, variable names, etc. as they go through more and more shared code. This means that the rest of the scripts should be more equivalent. 
...
For mice/3D data (x and y over time), this is fairly tractable with some downsampling. Specifically We use [NeuroPattToolbox](https://github.com/BrainDynamicsUSYD/NeuroPattToolbox) from the Brain Dynamics Group to delineate activity movement trajectories in our mouse data. More info on this software in the table at the end of this markdown. Make sure you also check out [some](https://www.nature.com/articles/s41562-023-01628-3) [of](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1006643) [their](https://www.jneurosci.org/content/41/16/3665) [papers](https://pmc.ncbi.nlm.nih.gov/articles/PMC10015011/) if you are interested in their tools. 
...
### 2A. Running optical flow
  ...
  OpFl LSD: This [script](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/OpFl_lsd.m) is also equivalent. Helper scripts are [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/sbatch_OpFl_lsd.sh) and [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/sbatch_lsd_all.sh).
...
### 2B. Magnitudes
  ...
  For the sake of convenience, note that these extractions (2B and 2C) are integrated into [sbatch_OpFl.sh](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/sbatch_OpFl.sh) and equivalent scripts for studies 2 and 3. If you want to run analyses broadly, it might be easier to use these pipeline-level scripts rather than run extractions individually for each instance.
  ...
  *Aggregating DMN magnitudes for assessment of session-to-session differences*
  ...
  For human LSD, we'll use this [script](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Extract_DMNMag_lsd.m).
  
  For mouse LSD, we'll use this [Extract_mag_dif_mice.m](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/mice/Extract_mag_dif_mice.m). For LSD there's one mouse missing a 6th session, but it's not as heterogeneously populated as the psilocybin data.
...
### 2C. Bottom-Up Relative Angles
  ...
  Extract Relative angles LSD: Also [equivalent](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Extract_RelativeAngles_lsd.m). 
...
*Contrast maps are generated for figures 2J, 2K, 3J, 3K, S7, and S8.*

  To calculate the placebo - drug and no drug - drug contrasts, we'll have to run two other scripts first. The first script averages magnitudes for each subject (MDMA [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Calc_AvgMagnitude.m), psilocybin [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Calc_AvgMagnitude_psil.m). After calculating these values, we can find the average across subjects/sessions with the AvgMag_x_L/R.mat output files generated by using [this script](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Aggregate_AvgMags.m) for MDMA and [this script](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Aggregate_AvgMag_psil.m) for psilocybin. LSD is done in one shot because of it's more wieldly file structure [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Extract_faceMag_dif_lsd.m). These last scripts will print out the images found in figure 2 using [Vis_FaceVec_Mag](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Vis_FaceVec_Mag.m)

  A similar  procecdure can be applied to generate contrasts maps for aggregate directionality shifts in cortical flow. Extract relative angles provides the output needed for [Extract_rho_dif]([https://github.com/WilliamsPanLab/PsychedProps/blob/a95be2b4c4c29cbbb93ae411fc31911cdba23ab0/scripts/Calc_AvgBup.m](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Extract_rho_dif.m) (MDMA) and [Extract_rho_dif_psil](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Extract_rho_dif_psil.m) and [LSD](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Extract_rho_dif_lsd.m). After calculating these values, we aggregate the outputs with R for [left](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/Mod_rhos_humans.R#L6) and [right](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/Mod_rhos_humans_R.R#L6) hemispheres. These outputs are then plotted with [this script](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/Make_DMNDIR_dif_png.m) for *Figure 3J*.
...
**3A.I** Humans
  [Extract_DMNSeg](https://github.com/WilliamsPanLab/PsychedProps/blob/ccac39b7736991f33c2027a2964eb9d968369a55/scripts/Extract_DMNSeg.m) just takes the time series, applies the same masking used in the optical flow pipeline (in terms of the DMN and temporal masking), and calculates the functional connectivity between the DMN and the rest of the cortex. [Here](https://github.com/WilliamsPanLab/PsychedProps/blob/ccac39b7736991f33c2027a2964eb9d968369a55/scripts/Extract_DMNSeg_psil.m) is the psilocybin version (different filepaths). The easiest way to use these is to uncomment them in the helper scripts [sbatch_OpFl.sh](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/sbatch_OpFl.sh) and [sbatch_OpFl_psil.sh](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/sbatch_OpFl_psil.sh). See [parent LSD script](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/sbatch_OpFl_lsd.sh) for study 3, which uses an equivalent [DMN Seg](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/Extract_DMNSeg_lsd.m) script.
  
  The scripts to aggregate the derived FC values is [Extract_DMNSeg_dif.m](https://github.com/WilliamsPanLab/PsychedProps/blob/ccac39b7736991f33c2027a2964eb9d968369a55/scripts/Extract_DMNSeg_dif.m) for MDMA, [Extract_DMNSeg_dif_psil.m](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Extract_DMNSeg_dif_psil.m) for psilocybin, and [this script](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/Extract_DMNSeg_dif_lsd.m) for human lsd. The easiest way to run these (on Sherlock if you are at Stanford) is just using an interactive node and running it through command-line matlab. Runs very quickly.
...
**3B.I** Humans
  We're following the lead of [this](https://www.nature.com/articles/s41593-023-01299-3) paper because they put forward a case that a lot of effects can be simplified to changes in temporal autocorrelation. The operationalization of autocorrelation essentially comes down to the correlation between signal and signal shifted "+1" in time. The script to calculate temporal autocorrelation in the DMN for MDMA is [Extract_TAutoCor.m](https://github.com/WilliamsPanLab/PsychedProps/blob/ccac39b7736991f33c2027a2964eb9d968369a55/scripts/Extract_TAutoCor.m), for psilocybin it is [Extract_TAutoCor_psil.m](https://github.com/WilliamsPanLab/PsychedProps/blob/ccac39b7736991f33c2027a2964eb9d968369a55/scripts/Extract_TAutoCor_psil.m), and for LSD it is [this script](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/Extract_TAutoCor_lsd.m). To aggregate the MDMA autocor values, use [Extract_TA_dif.m](https://github.com/WilliamsPanLab/PsychedProps/blob/ccac39b7736991f33c2027a2964eb9d968369a55/scripts/Extract_TA_dif.m). To aggregate the psilocybin autocor values, use the [Extract_TA_dif_psil.m](https://github.com/WilliamsPanLab/PsychedProps/blob/ccac39b7736991f33c2027a2964eb9d968369a55/scripts/Extract_TA_dif_psil.m) script, and to aggregate human LSD autocor values, use [Extract_TA_dif_lsd.m](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/Extract_TA_dif_lsd.m).
...
- **4B.III** LSD (humans)
  Post-aggregation DMN integration evaluation in study 3 is within the master [LSD.rmd](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/LSD.Rmd) script.
- **4B.IV** LSD (mice)
  Post-aggregation DMN integration evaluation in study 3 (mice/LSD) is within the bottom-up analysis script (4D.IV).
...
- **4B.III** LSD (humans)
  Post-aggregation DMN autocorrelation evaluation in study 3 is within the master [LSD.rmd](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/LSD.Rmd) script.
- **4C.IV** LSD
  Post-aggregation DMN autocorrelation evaluation in study 4 (mice/LSD) is within the bottom-up analysis script (4D.III).

  ### 4D. Bottom-Up Analysis
- **4D.I** MDMA
  Remember this script encapsulates DMN integration/segregation and autocorrelation scripting. It can be found in markdown format [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz.md), or as an .rmd [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz.Rmd). *Figures for 1A, 3A, 3B, 4A, 4E, 4F, 4I, and 4J are generated within. Figures S5 and S10 are also generated in this script* 
- **4D.II** Psilocybin
  The equivalent script for psilocybin can be found [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz_psil.md). Note this also has the lasting effects lil' chunk of code (just after line 1,050). The .rmd version is [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz_psil.Rmd). *Figures for 1B, 3C, 3D, 3L, 4B, 4B, S9, and S10 are generated within*
- **4D.III** LSD (humans)
  As stated above, as R-generated human LSD figures are within [LSD.rmd](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/LSD.Rmd) *Figures for 1C, 3E, 4C, and S10*
- **4D.IV** LSD (mice)
  The equivalent knit markdown for mice can be found [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_viz_mice.md). The .rmd version can be found [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_viz_mice.Rmd). *Figures for 1D, 3F, 3G, 3H, 3I, 4D, and S10 are generated within*


## 5. Bootstraps and AUC curves
You can find the bootstrap and AUC analyses for MDMA (figure 4) further down the [same markdown file used above](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz.md). The .md file doesn't have line numbers, but you can cntrl-F to "library(pROC)" to find where this section stats. If instead you are looking at [the .Rmd](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz.Rmd), this starts at line 1029.

Same goes for bootstrap and AUC/bootstraps for psilocybin. Check out the [same markdown as prior](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz_psil.md). Corresponding .rmd is [here](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz_psil.Rmd), and AUC stuff also starts at line 1149. For human [LSD](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/LSD.Rmd), it starts at line 384.
...
## 6. Self-Report
You can find the bootstrap and AUC analyses (figure 4) further down the [same markdown file used above](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz.md). The .md file doesn't have line numbers, but you can cntrl-F to "inter-psychedelic-session to find where this section starts. If instead looking at the [.Rmd](https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz.Rmd), this starts at line 1350. For psilocybin, it starts at line [1455]((https://github.com/WilliamsPanLab/PsychedProps/blob/master/scripts/Stats_n_Viz_psil.Rmd). For human [LSD](https://github.com/WilliamsPanLab/PsychedProps/blob/82f5c7ef55ad19c0e4e26f0d6ef953d71cb1a13f/scripts/LSD.Rmd), it starts at line 443.

## 7. DMN specificity tests
DMN specificty tests fall into 2 categories. In Figure *S12*, the localization of psychedelic-propagation and psychedelic-correlation metrics are evaluated within the DMN versus 2,000 spun DMN maps. You'll have the re-run the pipleine using the _Spun suffix scripts, which exist for every step. In Figure *S13*, the localization of psychedelic-propagation and psychedelic-correlation metrics are evaluated within the other 3 networks (visual, motor, frontoparietal) versus 2,000 spun DMN maps. For these scripts, please locate the _VIS, _MOT, and _FPN suffixed scripts, as well as their corresponding _Spun measures. This is quite computationally intensive to re-run, taking >48 hours per subject even with multithreading to generate null distributions.
