...
for t=1:3
        ...
        imagefp=char(strjoin(['/scratch/users/apines/data/DES/' string(subj) '/' task '_lh.mgh'],''));
        imagefp_out=char(strjoin(['/scratch/users/apines/data/DES/' string(subj) '/' task '_lh_cent.mgh'],''));
        % load image
        ...
        %%% RIGHT
        % input image filepath
        imagefp=char(strjoin(['/scratch/users/apines/data/DES/' string(subj) '/' task '_rh.mgh'],''));
        imagefp_out=char(strjoin(['/scratch/users/apines/data/DES/' string(subj) '/' task '_rh_cent.mgh'],''));
        % load image
        image=MRIread(imagefp);
        % convert to true dimensionality
        series=squeeze(image.vol);
...
% AS FOR DS_SURF_TS, LEGACY SIDELINED FOR NOW UNTIL PROC'ED
% MAKE SURE TO CHANGE FILTER IN ACCORDANCE WITH LEGACY TR WHEN #RUNNING
% F=1/LegacyTR_r;
% nyquist
% Ny=F/2;
% lower and upper limit
