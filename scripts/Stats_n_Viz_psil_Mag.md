...
![](Stats_n_Viz_psil_Mag_files/figure-gfm/unnamed-chunk-10-1.png)<!-- -->
...
saveRDS(allScans,'~/Downloads/Psil_MagMerged.rds')
```
...
![](Stats_n_Viz_psil_Mag_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
...
# gaussian jitter, as.numeric, -1 because none is retained as a possible factor level despite not being populated
set.seed(1)
...
![](Stats_n_Viz_psil_Mag_files/figure-gfm/unnamed-chunk-16-1.png)<!-- -->
...
ggplot(allScansNoMeth, aes(x = Condition2, y = ResidualsReduced)) +
  ...
  labs(title = "Psilocybin: Lasting effects \n",
       x = "",
       y = "Avg. Magnitude in DMN") + scale_x_discrete(labels=c('No Drug','Post-Psil','During Psil'))+
  theme_minimal(base_size=25)+scale_color_manual(values = generated_colors)
```

![](Stats_n_Viz_psil_Mag_files/figure-gfm/unnamed-chunk-17-1.png)<!-- -->
