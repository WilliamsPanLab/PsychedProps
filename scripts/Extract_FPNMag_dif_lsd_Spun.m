...
for t =1:3
	...
	for s=1:20;
		...
		SubjNameCol((s+(t*20))-20,2)=cellstr(num2str(t));
	        % grab PCB
		bvFP=[commonFP subjList(s) '/' subjList(s) '_PCB_' task '_FPNMag_Spun.csv'];
	        bvFP=strjoin(bvFP,'');
		% m1 = after
		m1FP=[commonFP subjList(s) '/' subjList(s) '_LSD_' task '_FPNMag_Spun.csv'];
	        m1FP=strjoin(m1FP,'');
...
% save out matrix
writetable(table(outDF),['/scratch/users/apines/data/lsd_FPNMagMerged_Spin_' num2str(n) '.csv'])
