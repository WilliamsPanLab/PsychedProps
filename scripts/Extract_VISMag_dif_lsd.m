...
for t =1:3
	...
	for s=1:20;
		...
		SubjNameCol((s+(t*20))-20,2)=cellstr(num2str(t));
	        % grab PCB
		bvFP=[commonFP subjList(s) '/' subjList(s) '_PCB_' task '_VISMag.csv'];
	        bvFP=strjoin(bvFP,'');
		% m1 = after
		m1FP=[commonFP subjList(s) '/' subjList(s) '_LSD_' task '_VISMag.csv'];
	        m1FP=strjoin(m1FP,'');
...
% save out matrix
writetable(table(outDF),['/oak/stanford/groups/leanew1/users/apines/data/lsd_VISMagMerged.csv'])
