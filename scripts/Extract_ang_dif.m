...
for task=["rs1" "rs2" "emotion" "gambling" "wm"]
	...
	for s=[1 2 3 5 7 8 9 11 12 13 14 15 16 17]
		disp(s)
	        ...
	        bvFP=[commonFP subjList(s) '/' seshInfo{1} '/' subjList(s) '_' seshInfo{1} '_' task '_k4_Prop_Feats_ind.csv'];
	        bvFP=strjoin(bvFP,'');
		% and grab reverse phase encoding direction nifti
		pFP=[commonFP subjList(s) '/' seshInfo{2} '/' subjList(s) '_' seshInfo{2} '_' task '_k4_Prop_Feats_ind.csv'];
	        pFP=strjoin(pFP,'');
	        m1FP=[commonFP subjList(s) '/' seshInfo{3} '/' subjList(s) '_' seshInfo{3} '_' task '_k4_Prop_Feats_ind.csv'];
	        m1FP=strjoin(m1FP,'');
	        m2FP=[commonFP subjList(s) '/' seshInfo{4} '/' subjList(s) '_' seshInfo{4} '_' task '_k4_Prop_Feats_ind.csv'];
	        m2FP=strjoin(m2FP,'');
