function OpFlStreamlines_Ind(s)
% create streamline figures for one subject 

% read in subject dosage correspondence, has to be before library addpath for some silly reason
subSeshDose=readtable('~/subjSeshDoseCorresp.csv');
% DO ABOVE IN PARENT SCRIPT AND ENTER SUB SESH DOSE AS ITS OWN ARGUMENT
...
% unlike _gro script, s is fed in as an argument here
...
% get "mean" (nomenclature more sensibly applied to group script) for this PT
bv_g_angles_meanx=bv_g_anglesx(s,:);
p_g_angles_meanx=p_g_anglesx(s,:);
m1_g_angles_meanx=m1_g_anglesx(s,:);
m2_g_angles_meanx=m2_g_anglesx(s,:);
% y components
bv_g_angles_meany=bv_g_anglesy(s,:);
p_g_angles_meany=p_g_anglesy(s,:);
m1_g_angles_meany=m1_g_anglesy(s,:);
m2_g_angles_meany=m2_g_anglesy(s,:);
...
print(['~/streams/' num2str(s) 'BV.png'],'-dpng')
%%%%%%%%%%%%%%
% placebo
% make x y rhos three rows in a matrix
vectors_s=zeros(3,5120);
...
print(['~/streams/' num2str(s) 'P.png'],'-dpng')
%%%%%%%%%%%%%%
% 80 mg
%%%%%%%%%%%%%%
% make x y rhos three rows in a matrix
...
print(['~/streams/' num2str(s) 'm1.png'],'-dpng')
%%%%%%%%%%%%%%
% 120 mg
%%%%%%%%%%%%%%
% make x y rhos three rows in a matrix
...
print(['~/streams/' num2str(s) 'm2.png'],'-dpng')

colorbar

print(['~/streams/' num2str(s) 'm2cb.png'],'-dpng')
...
view(230, 185);
daspect([1 1 1]);
print(['~/streams/' num2str(s) 'nond_mind.png'],'-dpng')
