...
for t =1:3
	...
	for s=1:20;
		...
		SubjNameCol((s+(t*20))-20,2)=cellstr(num2str(t));
	        % grab PCB
		bvFP=[commonFP subjList(s) '/' subjList(s) '_PCB_' task '_TemporalAutoCor_FPN.csv'];
	        bvFP=strjoin(bvFP,'');
		% m1 = after
		m1FP=[commonFP subjList(s) '/' subjList(s) '_LSD_' task '_TemporalAutoCor_FPN.csv'];
	        m1FP=strjoin(m1FP,'');
...
% save out matrix
writetable(table(outDF),['/oak/stanford/groups/leanew1/users/apines/data/lsd_TAMerged_FPN.csv'])
% and subject ID column
writetable(table(SubjNameCol),['/oak/stanford/groups/leanew1/users/apines/data/lsd_TAMerged_subjOrder_FPN.csv'])
