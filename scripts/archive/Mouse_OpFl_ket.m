...
basefp='/scratch/users/apines/p50_mice/proc/20200310/';
% load in specified scan
if run==1	
	fn = [basefp 'thy1gc6s_25mgkg_' subj '_preKet25mgkg_1/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
...
% post 1
if run==2
	fn = [basefp 'thy1gc6s_25mgkg_' subj '_postKet25mgkg_0/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
	else
...
% post 2
if run==3	
	fn = [basefp 'thy1gc6s_25mgkg_' subj '_postKet25mgkg_5/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
	else    
...
% post 3
if run==4
	fn = [basefp 'thy1gc6s_25mgkg_' subj '_postKet25mgkg_10/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
	else
...
% post 4
if run==5
	fn = [basefp 'thy1gc6s_25mgkg_' subj '_postKet25mgkg_15/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
	else
