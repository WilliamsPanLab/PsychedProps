...
for (f in 1:Lfaces){
	...
	mmodel=lme(DMNProp ~ Drug + FD + RemTRs, random = ~ 1 | SubjID, data = ctmasterdf)
	# extract t stat
	T_L[f]=summary(mmodel)$tTable[2,4]
	p_L[f]=summary(mmodel)$tTable[2,5]
}
# write out t stats
write.csv(T_L,paste0('/scratch/users/apines/DMN/DMN_Prop_Tstats_L_Drug.csv'))
write.csv(p_L,paste0('/scratch/users/apines/DMN/DMN_Prop_ps_L_Drug.csv'))
...
for (f in 1:Rfaces){
	...
	mmodel=lme(DMNProp ~ Drug + FD + RemTRs, random = ~ 1 | SubjID, data = ctmasterdf)
	# extract t stat
	T_R[f]=summary(mmodel)$tTable[2,4]
	p_R[f]=summary(mmodel)$tTable[2,5]
}
# write out t stats and ps
write.csv(T_R,paste0('/oak/stanford/groups/leanew1/users/apines/results/DMNProps_Tstats_R_Drug.csv'))
write.csv(p_R,paste0('/oak/stanford/groups/leanew1/users/apines/results/DMNProps_ps_R_Drug.csv'))

# do FDR correcting internally on combined p values
ps_combined=c(p_L,p_R)
ps_combined_fdr=p.adjust(ps_combined,method='fdr')
# re-extract left and right hemisphere corrected values
p_L=ps_combined_fdr[1:length(p_L)]
p_R=ps_combined_fdr[(length(p_L)+1):length(ps_combined_fdr)]
# mask t values by significant at FDR <0.05
T_L_masked=T_L
T_L_masked[p_L>0.05]=0
T_R_masked=T_R
T_R_masked[p_R>0.05]=0
# writeout 
write.csv(T_L_masked,paste0('/oak/stanford/groups/leanew1/users/apines/results/DMNProps_Tstats_L_Drug_masked.csv'))
write.csv(T_R_masked,paste0('/oak/stanford/groups/leanew1/users/apines/results/DMNProps_Tstats_R_Drug_masked.csv'))
