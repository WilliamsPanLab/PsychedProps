function Extract_BUTD_ResultantVecs_PL(infileOpFl,infileAngD,outfile_L,outfile_R)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Angs_L=Thetas_L;
L_CM=circ_mean(Angs_L);
VL_L=circ_r(Angs_L);
[HorzC_L,VertC_L]=pol2cart(L_CM,VL_L);
if (std(FaceAngDistPGG_L)~=0)
        OutDf_L(F,2)=num2cell(HorzC_L);
        OutDf_L(F,3)=num2cell(VertC_L);
end
...
%%%%%%% granular feature extraction
Angs_R=Thetas_R;
R_CM=circ_mean(Angs_R);
VL_R=circ_r(Angs_R);
[HorzC_R,VertC_R]=pol2cart(R_CM,VL_R);
if (std(FaceAngDistPGG_R)~=0)
	OutDf_R(F,2)=num2cell(HorzC_R);
	OutDf_R(F,3)=num2cell(VertC_R);
end
...
%%% save output df
save(outfile_L,'OutDf_L')
save(outfile_R,'OutDf_R')
