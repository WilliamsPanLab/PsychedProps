...
for t =1:3
	...
	for s=1:20;
		...
		SubjNameCol((s+(t*20))-20,2)=cellstr(num2str(t));
	        % grab PCB
		bvFP=[commonFP subjList(s) '/' subjList(s) '_PCB_' task '_k1_SDS_L.csv'];
	        bvFP=strjoin(bvFP,'');
		% right hemisphere
		bvRFP=[commonFP subjList(s) '/' subjList(s) '_PCB_' task '_k1_SDS_R.csv'];
	        bvRFP=strjoin(bvRFP,'');
		% m1 = after
		m1FP=[commonFP subjList(s) '/' subjList(s) '_LSD_' task '_k1_SDS_L.csv'];
	        m1FP=strjoin(m1FP,'');
		% m1 right hemisphere
		m1RFP=[commonFP subjList(s) '/' subjList(s) '_LSD_' task '_k1_SDS_R.csv'];
	        m1RFP=strjoin(m1RFP,'');
	...
	writematrix(thetas_L_PCB,strjoin(['/oak/stanford/groups/leanew1/users/apines/data/lsd_SDS_' task '_PCB_L.csv'],''));
	writematrix(thetas_L_LSD,strjoin(['/oak/stanford/groups/leanew1/users/apines/data/lsd_SDS_' task '_LSD_L.csv'],''));
	writematrix(thetas_R_PCB,strjoin(['/oak/stanford/groups/leanew1/users/apines/data/lsd_SDS_' task '_PCB_R.csv'],''));
	writematrix(thetas_R_LSD,strjoin(['/oak/stanford/groups/leanew1/users/apines/data/lsd_SDS_' task '_LSD_R.csv'],''));
end
