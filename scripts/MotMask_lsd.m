function MotMask_lsd(subj,sesh,task)
...
if isfile(fpL)
	...
	numTRs=tsSize(2);
	%%% load in mask
	confFilePath=['/scratch/users/apines/LSD_ICL/rest_proc/' subj '_' sesh '_bold_signal_properties.mat'];
	...
	if length(FD)~=220
		error('you sure about that? check that bold/.mat. Not 220 length.')
	end
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	Tmask=conf1.tmask(scanInds);
