...
for task=["rs1" "rs2" "emotion" "gambling" "wm"]
	...
	for s=[1 2 3 5 7 8 9 11 12 13 14 15 16 17]
		disp(s)
	        ...
	        bvFP=[commonFP subjList(s) '/' seshInfo{1} '/' subjList(s) '_' seshInfo{1} '_' task '_DMNMags_L.csv'];
	        bvFP=strjoin(bvFP,'');
		% and grab reverse phase encoding direction nifti
		pFP=[commonFP subjList(s) '/' seshInfo{2} '/' subjList(s) '_' seshInfo{2} '_' task '_DMNMags_L.csv'];
	        pFP=strjoin(pFP,'');
	        m1FP=[commonFP subjList(s) '/' seshInfo{3} '/' subjList(s) '_' seshInfo{3} '_' task '_DMNMags_L.csv'];
	        m1FP=strjoin(m1FP,'');
	        m2FP=[commonFP subjList(s) '/' seshInfo{4} '/' subjList(s) '_' seshInfo{4} '_' task '_DMNMags_L.csv'];
	        m2FP=strjoin(m2FP,'');
		% and right hemisphere fps
		bvRFP=[commonFP subjList(s) '/' seshInfo{1} '/' subjList(s) '_' seshInfo{1} '_' task '_DMNMags_R.csv'];
	        bvRFP=strjoin(bvRFP,'');
		pRFP=[commonFP subjList(s) '/' seshInfo{2} '/' subjList(s) '_' seshInfo{2} '_' task '_DMNMags_R.csv'];
		pRFP=strjoin(pRFP,'');
		m1RFP=[commonFP subjList(s) '/' seshInfo{3} '/' subjList(s) '_' seshInfo{3} '_' task '_DMNMags_R.csv'];
		m1RFP=strjoin(m1RFP,'');
		m2RFP=[commonFP subjList(s) '/' seshInfo{4} '/' subjList(s) '_' seshInfo{4} '_' task '_DMNMags_R.csv'];
		m2RFP=strjoin(m2RFP,'');
	...
	writematrix(round(thetas_L_bv, 5), strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_Mags_L_bv.csv'], ''))
	writematrix(round(thetas_L_pcb, 5), strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_Mags_L_pcb.csv'], ''))
	writematrix(round(thetas_L_m1, 5), strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_Mags_L_m1.csv'], ''))
	writematrix(round(thetas_L_m2, 5), strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_Mags_L_m2.csv'], ''))
	writematrix(round(thetas_R_bv, 5), strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_Mags_R_bv.csv'], ''))
	writematrix(round(thetas_R_pcb, 5), strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_Mags_R_pcb.csv'], ''))
	writematrix(round(thetas_R_m1, 5), strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_Mags_R_m1.csv'], ''))
	writematrix(round(thetas_R_m2, 5), strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_Mags_R_m2.csv'], ''))
end
