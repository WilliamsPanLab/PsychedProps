...
for task=["rs1" "rs2" "emotion" "gambling" "wm"]
	...
	for s=[1 2 3 5 7 8 9 11 12 13 14 15 16 17]
		...
		bvFP=[commonFP subjList(s) '/' seshInfo{1} '/' subjList(s) '_' seshInfo{1} '_' task '_DMNMag.csv'];
		bvFP=strjoin(bvFP,'');
		bv=readmatrix(bvFP);
	        outDF(s,1)=bv(1,2);
		...
		if exist(pFP,'file')
		% check using props, but load in Dseg
                pFP=[commonFP subjList(s) '/' seshInfo{2} '/' subjList(s) '_' seshInfo{2} '_' task '_DMNMag.csv'];
                pFP=strjoin(pFP,'');
		p=readmatrix(pFP);
		...
		if exist(m1FP,'file')
		% check using props, but load in Dseg
                m1FP=[commonFP subjList(s) '/' seshInfo{3} '/' subjList(s) '_' seshInfo{3} '_' task '_DMNMag.csv'];
                m1FP=strjoin(m1FP,'');
		m1=readmatrix(m1FP);
		...
		if exist(m2FP,'file')
		% check using props, but load in Dseg
		m2FP=[commonFP subjList(s) '/' seshInfo{4} '/' subjList(s) '_' seshInfo{4} '_' task '_DMNMag.csv'];
                m2FP=strjoin(m2FP,'');
		m2=readmatrix(m2FP);
	...
	% save out matrix
	writematrix(outDF,strjoin(['/oak/stanford/groups/leanew1/users/apines/data/' task '_DMNMagMerged.csv'],''))
end
