...
# make donut plot
DMNAngs_long$Drug<-as.factor(DMNAngs_long$Drug)
donutData<- data.frame(
  Category=levels(DMNAngs_long$Drug),
  count=tabulate(DMNAngs_long$Drug)
)

# convert labels to be consistent across studies
donutData$Category=c('No Drug','Psychedelic')
...
ggplot(donutData, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Category)) +
  ...
  )+guides(fill = guide_legend(title = NULL))+scale_fill_manual(values = c("#0018A8","#9532AA"))
