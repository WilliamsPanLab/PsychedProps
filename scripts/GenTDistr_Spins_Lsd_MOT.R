...
for (n in 1:numSpins){
	LSD=read.csv(paste0('/scratch/users/apines/data/lsd_propsMerged_Spin_',n,'_MOT.csv'))
	Thresh=200
...
print('done with BUP')
for (n in 1:numSpins){
        LSD=read.csv(paste0('/scratch/users/apines/data/lsd_MOTMagMerged_Spin_',n,'.csv'))
        LSD=LSD[LSD$outDF_5>Thresh,]
        LSD=LSD[!is.na(LSD$outDF_3),]
        colnames(LSD)<-c('subj','sesh','PercBUP','task','RemainingTRs','meanFD')
        LSD$sesh <- relevel(as.factor(LSD$sesh), ref = "PCB")
...
print('done with mag')
for (n in 1:numSpins){
        LSD=read.csv(paste0('/scratch/users/apines/data/lsd_MOTSegMerged_Spin_',n,'.csv'))
        LSD=LSD[LSD$outDF_5>Thresh,]
        LSD=LSD[!is.na(LSD$outDF_3),]
        colnames(LSD)<-c('subj','sesh','PercBUP','task','RemainingTRs','meanFD')
        LSD$sesh <- relevel(as.factor(LSD$sesh), ref = "PCB")
...
print('done with seg')
for (n in 1:numSpins){
        LSD=read.csv(paste0('/scratch/users/apines/data/lsd_TAMerged_Spin_',n,'_MOT.csv'))
        LSD=LSD[LSD$outDF_5>Thresh,]
        LSD=LSD[!is.na(LSD$outDF_3),]
        colnames(LSD)<-c('subj','sesh','PercBUP','task','RemainingTRs','meanFD')
        LSD$sesh <- relevel(as.factor(LSD$sesh), ref = "PCB")
...
saveRDS(outspins,'~/SpunTDistributions_LSD_MOT.rds')
