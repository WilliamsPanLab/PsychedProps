...
ggplot(mergedDf_clean, aes(x = Drug, y = Residuals)) +
  ...
  labs(title = "MDMA vs. Placebo \n",
       x = "",
       y = "Avg. Magnitude in DMN") + scale_x_discrete(labels=c('Placebo','MDMA'))+
...
ggplot(mergedDf, aes(x = Drug, y = Residuals)) +
  ...
  labs(title = "MDMA vs. No-drug scans \n",
       x = "",
       y = "Avg. Magnitude in DMN") + scale_x_discrete(labels=c('No Drug','MDMA'))+
...
bootstrap_results_FD=rbind(td_fd,ta_fd,cx_fd,ds_fd,dm_fd)
bootstrap_results_Drug=rbind(td_d,ta_d,cx_d,ds_d,dm_d)
# Create box-and-whisker plots
ggplot(bootstrap_results_FD, aes(x = Model, y = tstat, fill = Cov)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  labs(x = "Model", y = "T-Values", title = "Bootstrap T-Values for MeanFD") +
  theme_minimal(base_size = 16)#+scale_fill_manual(values=c('blue','red'))

ggplot(bootstrap_results_Drug, aes(x = Model, y = tstat, fill = Cov)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  labs(x = "Model", y = "T-Values", title = "Bootstrap T-Values for Drug") +
  theme_minimal(base_size = 16)#+scale_fill_manual(values=c('blue','red'))
...
```{r}
#### These are mediations! construe as such
...
directEffects <- NULL
indirectEffects <- NULL
totalEffects <- NULL
directPvalues <- NULL
indirectPvalues <- NULL
totalPvalues <- NULL
# dummy variables instead of multi-category task variable
VASmerge$Gambling=0
VASmerge$WM=0
VASmerge$Gambling[VASmerge$Task=='gambling']=1
VASmerge$WM[VASmerge$Task=='wm']=1
VASmerge$Gambling<-as.factor(VASmerge$Gambling)
VASmerge$WM<-as.factor(VASmerge$WM)
# Loop through each DAS item
for (i in 40:56) {
  das_col <- colnames(VASmerge)[i]
  print(das_col)
  # standardize self report item to z-score
  VASmerge$Zed<-scale(VASmerge[,das_col])
  # Define the SEM model with the current DAS item
  sem_model <- paste0('
    TDProp1z ~ a*Drug + MeanFD + RemTRs + WM + Gambling
    Zed ~ c*Drug + MeanFD + RemTRs + WM + Gambling + b*TDProp1z
  ...
  # Fit the model
  fit <- sem(sem_model, data = VASmerge, missing = "fiml")
  
  # Extract results
  estimates <- parameterEstimates(fit, standardized = TRUE)
  directEffects <- c(directEffects, estimates$est[estimates$label == "direct"])
  indirectEffects <- c(indirectEffects, estimates$est[estimates$label == "indirect"])
  totalEffects <- c(totalEffects, estimates$est[estimates$label == "total"])
  directPvalues <- c(directPvalues, estimates$pvalue[estimates$label == "direct"])
  indirectPvalues <- c(indirectPvalues, estimates$pvalue[estimates$label == "indirect"])
  totalPvalues <- c(totalPvalues, estimates$pvalue[estimates$label == "total"])
...
results <- data.frame(
  DAS_Item = dascNames,
  Direct_Effect = directEffects,
  Indirect_Effect = indirectEffects,
  Total_Effect = totalEffects,
  Direct_Pvalue = directPvalues,
  Indirect_Pvalue = indirectPvalues,
  Total_Pvalue = totalPvalues
)
