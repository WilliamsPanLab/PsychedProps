...
sesh = sys.argv[2]
# output fp
pofp = '/scratch/users/apines/' + str(subj) + '_' + str(sesh) + '2.png'
# prevent plot from trying to render midscript
matplotlib.use('agg')
# set resolution
plt.rcParams['figure.dpi'] = 500
# load in data
TSfp='/scratch/users/apines/data/mdma/' + str(subj) + '/' + str(sesh) + '/' + str(subj) + '_' + str(sesh) + '_L_rs2_TS_3k.func.gii'
gif_CL=nb.load(TSfp)
CL=np.array(gif_CL.agg_data()[:])
TSfp='/scratch/users/apines/data/mdma/' + str(subj) + '/' + str(sesh) + '/' + str(subj) + '_' + str(sesh) + '_R_rs2_TS_3k.func.gii'
gif_CR=nb.load(TSfp)
...
rs1ConfTsv= xcpd_outdir + subj + '_' + sesh + '_task-rs_acq-mb_dir-pe1_run-0_motion.tsv'
...
fig, axes = plt.subplots(len(lobe_data)+1, 1, figsize=(10, 2 * len(lobe_data)),
                         sharex=True, gridspec_kw={'height_ratios': [1] + [4]*len(lobe_data)})
axes[0].plot(FD, color='red', linewidth=0.8)
axes[0].axhline(0.2, color='black', linestyle='--', linewidth=0.5)
axes[0].set_ylabel('FD')
axes[0].set_title(f'{subj}_{sesh} Lobe-wise Sorted TS with FD')
