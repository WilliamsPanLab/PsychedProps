...
ggplot(donutData, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Category)) +
     geom_rect() +
     coord_polar(theta="y") + 
     xlim(c(1, 4)) + theme_classic(base_size=25)+theme(
    axis.text.y = element_blank(),
    axis.ticks = element_blank(),
    axis.line = element_blank()
  )+guides(fill = guide_legend(title = NULL))+scale_fill_manual(values = c("#0018A8","#FFB200","#9532AA"))
...
```{r}
# remove data that needs to be removed (subs 6 and 10, <250 TRs)
mergedDfPropsComplAutoCdmnFC=mergedDfPropsComplAutoCdmnFC[mergedDfPropsComplAutoCdmnFC$Subjects!='sub-MDMA006',]
mergedDfPropsComplAutoCdmnFC=mergedDfPropsComplAutoCdmnFC[mergedDfPropsComplAutoCdmnFC$Subjects!='sub-MDMA010',]
mergedDfPropsComplAutoCdmnFC=mergedDfPropsComplAutoCdmnFC[mergedDfPropsComplAutoCdmnFC$RemTRs>250,]

# change rs2 to rs for accurate task-modeling
mergedDfPropsComplAutoCdmnFC$Task[mergedDfPropsComplAutoCdmnFC$Task=='rs2']='rs'
mergedDfPropsComplAutoCdmnFC$Task=as.factor(mergedDfPropsComplAutoCdmnFC$Task)

# set rs to reference level
mergedDfPropsComplAutoCdmnFC <- within(mergedDfPropsComplAutoCdmnFC, Task <- relevel(Task, ref = 2))

# model
td_model <- lme(TDProp1 ~ MeanFD + Drug+RemTRs, random = ~ 1 | Subjects, data = mergedDfPropsComplAutoCdmnFC)
ta_model <- lme(AutoCor ~ MeanFD + Drug+RemTRs, random = ~ 1 | Subjects, data = mergedDfPropsComplAutoCdmnFC)
cx_model <- lme(Complexity ~ MeanFD + Drug+RemTRs, random = ~ 1 | Subjects, data = mergedDfPropsComplAutoCdmnFC)
dmnseg_model <- lme(DMNFC ~ MeanFD + Drug+RemTRs, random = ~ 1 | Subjects, data = mergedDfPropsComplAutoCdmnFC)

# for correlation matrix
forCormat=data.frame(mergedDfPropsComplAutoCdmnFC$TDProp1,mergedDfPropsComplAutoCdmnFC$AutoCor,mergedDfPropsComplAutoCdmnFC$Complexity,mergedDfPropsComplAutoCdmnFC$DMNFC,mergedDfPropsComplAutoCdmnFC$MeanFD,mergedDfPropsComplAutoCdmnFC$RemTRs)

colnames(forCormat)=c('TDProp1','AutoCor','Complexity','DMNFC','MeanFD','RemTRs')

library(corrplot)
source("http://www.sthda.com/upload/rquery_cormat.r")

corrmatrix=rquery.cormat(forCormat,type="full")
corrplot(as.matrix(corrmatrix$r))
```
...
# preproc for plot
mergedDfPropsComplAutoCdmnFC$Drug=0
mergedDfPropsComplAutoCdmnFC$Drug[mergedDfPropsComplAutoCdmnFC$Dosage=="120mg"]=1
mergedDfPropsComplAutoCdmnFC$Drug[mergedDfPropsComplAutoCdmnFC$Dosage=="80mg"]=1
mergedDfPropsComplAutoCdmnFC$Drug=as.factor(mergedDfPropsComplAutoCdmnFC$Drug)
mergedDfPropsComplAutoCdmnFC$Subjects<-as.factor(mergedDfPropsComplAutoCdmnFC$Subjects)
mergedDfPropsComplAutoCdmnFC$Dosage<-as.factor(mergedDfPropsComplAutoCdmnFC$Dosage)
# change rs2 to rs for accurate task-modeling
mergedDfPropsComplAutoCdmnFC$Task[mergedDfPropsComplAutoCdmnFC$Task=='rs2']='rs'
mergedDfPropsComplAutoCdmnFC$Task=as.factor(mergedDfPropsComplAutoCdmnFC$Task)
df_plot1=mergedDfPropsComplAutoCdmnFC[mergedDfPropsComplAutoCdmnFC$Dosage!="120mg",]
# for p-120
df_plot2=mergedDfPropsComplAutoCdmnFC[mergedDfPropsComplAutoCdmnFC$Dosage!="80mg",]
...
# by task
ggplot(mergedDfPropsComplAutoCdmnFC, aes(x = Task, y = Complexity)) +
...
ggplot(mergedDfPropsComplAutoCdmnFC,aes(x=Complexity,y=TDProp1))+geom_point()+theme_minimal(base_size=25)+ylab('Bottom-up %')
```
...
df_diff <- mergedDfPropsComplAutoCdmnFC %>%
  group_by(Subjects) %>%
...
for (i in 1:num_bootstrap_samples){
  # resample data
  data=mergedDfPropsComplAutoCdmnFC[sample(nrow(mergedDfPropsComplAutoCdmnFC), replace = TRUE), ]
  # fit on all models
  td_model <- lme(TDProp1 ~ MeanFD + Drug+RemTRs+Task+Complexity+AutoCor, random = ~ 1 | Subjects, data = data)
  ta_model <- lme(AutoCor ~ MeanFD + Drug+RemTRs+Task+Complexity+TDProp1, random = ~ 1 | Subjects, data = data)
  cx_model <- lme(Complexity ~ MeanFD + Drug+RemTRs+Task+TDProp1+AutoCor, random = ~ 1 | Subjects, data = data)
  ds_model <- lme(DMNFC ~ MeanFD + Drug+RemTRs+Task+TDProp1+AutoCor, random = ~ 1 | Subjects, data = data)
  # get t-values
...
bootstrap_results_FD=rbind(td_fd,ta_fd,cx_fd,ds_fd)
bootstrap_results_Drug=rbind(td_d,ta_d,cx_d,ds_d)
# Create box-and-whisker plots
ggplot(bootstrap_results_FD, aes(x = Model, y = abs(tstat), fill = Cov)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  labs(x = "Model", y = "T-Values", title = "Bootstrap T-Values for MeanFD") + ylim(c(-1,8))+
  theme_minimal(base_size = 16)#+scale_fill_manual(values=c('blue','red'))

ggplot(bootstrap_results_Drug, aes(x = Model, y = abs(tstat), fill = Cov)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  labs(x = "Model", y = "T-Values", title = "Bootstrap T-Values for Drug") + ylim(c(-1,8))+
  theme_minimal(base_size = 16)#+scale_fill_manual(values=c('blue','red'))
...
VASmerge=merge(vas,mergedDfPropsComplAutoCdmnFC,by=c("Subjects","Dosage"))
...
# merged
mergedAmyg=merge(AmygB,mergedDfPropsComplAutoCdmnFC,by='Subjects')
