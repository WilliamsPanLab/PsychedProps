...
```{r}
# prop angles
rs1=read.csv('~/rs1_propsMerged.csv',header=F)
rs2=read.csv('~/rs2_propsMerged.csv',header=F)
emo=read.csv('~/emotion_propsMerged.csv',header=F)
gambling=read.csv('~/gambling_propsMerged.csv',header=F)
wm=read.csv('~/wm_propsMerged.csv',header=F)
# set colnames
...
# model
model <- lme(TDProp1 ~ MeanFD + Drug + RemTRs, random = ~ 1 | Subjects, data = mergedDf)

# get head-motion regressed values for plotting
model_to_reg <- lm(TDProp1 ~ MeanFD + RemTRs, data = mergedDf)
mergedDf$Residuals<-model_to_reg$residuals
...
ggplot(df_plot1, aes(x = Drug, y = TDProp1, fill = Drug, group = SubjectTask, color = Subjects)) +
  ...
  ggtitle('') +
  theme_minimal(base_size = 22)+ylab(paste0('Degrees from Bottom-up'))+xlab('MDMA')

# with boxplots
# Creating separate datasets for box plots
boxplot_data_left <- df_plot1 %>%
  filter(Dosage == 'Placebo')

boxplot_data_right <- df_plot1 %>%
  filter(Dosage == '80mg')

boxplot_data_right2 <- df_plot2 %>%
  filter(Dosage == '120mg')

# combine drug data for box and whiskers
boxplot_data_right=rbind(boxplot_data_right,boxplot_data_right2)

# Plotting
ggplot() +
  # Scatter plot and lines for df_plot1
  geom_point(data = df_plot1, aes(x = as.numeric(Drug), y = TDProp1, group = SubjectTask, color = Subjects, alpha = 0.7), size = 4) +
  geom_line(data = df_plot1, aes(x = as.numeric(Drug), y = TDProp1, group = SubjectTask, color = Subjects)) +
  # Scatter plot and lines for df_plot2
  geom_point(data = df_plot2, aes(x = as.numeric(Drug), y = TDProp1, color = Subjects, alpha = 0.7), size = 4) +
  geom_line(data = df_plot2, aes(x = as.numeric(Drug), y = TDProp1, group = SubjectTask, color = Subjects)) +
  # Box plot on the left side
  geom_boxplot(data = boxplot_data_left, aes(x = as.numeric(Drug) - 0.25, y = TDProp1, group = Drug), width = 0.2, outlier.shape = NA, color = "black") +
  # Box plot on the right side
  geom_boxplot(data = boxplot_data_right, aes(x = as.numeric(Drug) + 0.25, y = TDProp1, group = Drug), width = 0.2, outlier.shape = NA, color = "black") +
  # Custom color scale
  scale_color_viridis_d(option = "plasma") +
  # Theme and labels
  ...
  scale_x_continuous(breaks = unique(as.numeric(df_plot1$Drug)), limits = c(min(as.numeric(df_plot1$Drug)) - 1, max(as.numeric(df_plot1$Drug)) + 1))

# regress out head motion for comparison plot
ggplot(df_plot1, aes(x = Drug, y = Residuals, fill = Drug, group = SubjectTask, color = Subjects)) +
  geom_point(size = 4, aes(alpha = 0.7)) +
  geom_line() +
  geom_point(data = df_plot2, aes(color = Subjects,alpha=.7), size = 4) +
  geom_line(data = df_plot2, aes(group = SubjectTask, color = Subjects)) +
  scale_color_viridis_d(option = "plasma") +
  ggtitle('FD regressed') +
  theme_minimal(base_size = 22)+ylab(paste0('Degrees from Bottom-up'))+xlab('MDMA')
...
circular_barplot(plot_df, "Coef_TDProp1", "Pval_TDProp1", "Pval_TDProp1_MC", "Coefficients of TDProp1")
