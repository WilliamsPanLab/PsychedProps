...
# Plotting
ggplot(df_plot1, aes(x = Drug, y = TDProp, fill = Drug, group = SubjectTask, color = Subjects)) +
...
# model
td_model <- lme(TDProp ~ MeanFD + Drug, random = ~ 1 | Subjects, data = mergedDfPropsComplAutoC)
...
for (i in 1:num_bootstrap_samples){
  ...
  # fit on all models
  td_model <- lme(TDProp ~ MeanFD + Drug, random = ~ 1 | Subjects, data = data)
...
bootstrap_results=rbind(td_d,td_fd,sa_d,sa_fd,cx_d,cx_fd)
...
for (i in 40:56){
  ...
  corvec[i-39] <- summary(currModel)$tTable[rownames(summary(currModel)$tTable) == "TDProp", "t-value"]
  pvec[i-39] <- summary(currModel)$tTable[rownames(summary(currModel)$tTable) == "TDProp", "p-value"]
...
for (i in 40:56){
  das_col <- colnames(VASmerge)[i]
  print(das_col)
  formula_str <- paste(das_col, "~ MeanFD + TDProp + Drug")
  df = na.omit(VASmerge[,c(das_col,'MeanFD','TDProp','Subjects','Drug')])
  currModel <- lme(as.formula(formula_str), random = ~ 1 | Subjects, data = df)
  corvec[i-39] <- summary(currModel)$tTable[rownames(summary(currModel)$tTable) == "TDProp", "t-value"]
