function SubCort_OpFl_hippo_L_psil(subj,sesh,task)
...
% caud L
caudBox_L_min = min(caud_L_coords,[],1)+1;
caudBox_L_max = max(caud_L_coords,[],1)-1;
% hippo R
...
% set hippo L mask
hippo_L_mask = false(size(data,1), size(data,2), size(data,3));
...
subcortVol = data(hippo_L_min(1):hippo_L_max(1), ...
...
% save outputs
outfp=[childfp '/' subj '_HippoL_vf_out_' sesh '_' task '.mat'];
save(outfp,'outputs');
