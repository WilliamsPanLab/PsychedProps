...
# initialize masterdf
drug<-'Init'
PowerInfo<-c(0,0.01408451,0.02816901,0.04225352,0.05633803,0.07042254,0.08450704,0.09859155,0.11267606,0.12676056,0.14084507,0.15492958,0.16901408,0.18309859,0.1971831,0.21126761,0.22535211,0.23943662,0.25352113,0.26760563,0.28169014,0.29577465,0.30985915,0.32394366,0.33802817,0.35211268,0.36619718,0.38028169,0.3943662,0.4084507,0.42253521,0.43661972,0.45070423,0.46478873,0.47887324,0.49295775,0.50704225,0.52112676,0.53521127,0.54929577,0.56338028,0.57746479,0.5915493,0.6056338,0.61971831,0.63380282,0.64788732,0.66197183,0.67605634,0.69014085,0.70422535)
Subject<-'Init'
scan<-'Init'
FD<-0.00
ROI<-'roi'
...
PowerDf=data.frame(matrix(nrow=1,ncol=55))
#### ADD FD
colnames(PowerDf)[1:4]=c('Drug','Subject','scan','ROI')
PowerDf[1,5:55]=PowerInfo
...
for (s in 1:length(subjvec)){
	...
	for (b in 1:length(Del_baselineFPs)){
		...
		for (r in 1:19){
			...
			# ADD FD
			PowerDf[RowCount,]=c('No',subj,task,ROIList[r],file[,r])
	...
	# for each drug csv
		...
		# determine scan type to populate scan type 
			task=strsplit(filename,'_')[[1]][2]
			# convert to vector and aggregate for this ROI
			PowerDf[RowCount,]=c('MDMA',subj,task,ROIList[r],file[,r])
			RowCount=RowCount+1
		}
        }
	# md tp2
