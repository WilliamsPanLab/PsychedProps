---
title: "Psilocybin Stats and Visualization: Study 2"
...
# note logic of "Session" numbering works between RDS's (TA, DMNMag, Seg, BUP), but not 100% corresponding to imaging filenames anymore due to temporal order logic ().
...
# plot one version without methyl conditions for supp text
...
```{r}
#### now load in info on days before/after scan for each subject

# get all "dates" files
date_files_dir <- "~/Downloads"
date_files <- list.files(date_files_dir, pattern = "_dates.csv$", full.names = TRUE)
# initialize df for data we want
all_dates <- data.frame(Label = character(), Date = numeric(), stringsAsFactors = FALSE)
# loop thru each file
for (f in date_files) {
  df<-read.csv(f)
  # just info we want
  df_subset <- df[, c("Label", "Date")]
  # append
  all_dates <- rbind(all_dates, df_subset)
}
# convert so we can look at post-psil timecourse
all_dates=all_dates[all_dates$Date>-1,]
# make a matching column
allScans$LabelMatch <- paste0(allScans$LegacySubjs,'_', allScans$Session)
# merge em
allScans_withDates <- merge(allScans, all_dates, by.x = "LabelMatch", by.y = "Label", all.x = TRUE)

# remove NAs
allScans_withDatesClean=allScans_withDates[!is.na(allScans_withDates$Date),]
allScans_withDatesClean$Date<-as.numeric(allScans_withDatesClean$Date)
allScans_withDatesClean$LegacySubjs<-as.factor(allScans_withDatesClean$LegacySubjs)
overTimeModel0=gam(TDProp1~RemTRs+FD+s(LegacySubjs,bs="re"), data = allScans_withDatesClean)
overTimeModel1=gam(TDProp1~s(Date,k=4)+RemTRs+FD+s(LegacySubjs,bs="re"), data = allScans_withDatesClean)
overTimeModel2=gam(TDProp1~RemTRs+FD+Psilo+s(LegacySubjs,bs="re"), data = allScans_withDatesClean)
overTimeModel3=gam(TDProp1~s(Date,k=4)+RemTRs+FD+Psilo+s(LegacySubjs,bs="re"), data = allScans_withDatesClean)
...
forCormat=data.frame(PsilMerged$BUP,PsilMerged$TmpAutoCor,PsilMerged$DMNFC,PsilMerged$FD,PsilMerged$RemTRs,PsilMerged$Magnitudes)

colnames(forCormat)=c('Bottom-Up%','AutoCor','DMNFC','MeanFD','RemTRs','DMNMag')
...
node_colors[3] <- "#c12139" 
node_colors[5:6] <- "#09416b"
node_colors[2] <- "#EF9500"
node_colors[4] <- "#EF9500"
# assign to graph
...
plot(g,
     ...
     main = "Measurement Correlations: Psil")
