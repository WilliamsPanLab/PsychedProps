...
# plot invididual scans to qc category assignment
ggplot(allScans, aes(x = TemporalOrder, y = TDProp1)) +
  geom_point(aes(color=Condition2,size=1)) +
  geom_vline(data = subset(allScans, Drug == 'Psilo'), aes(xintercept = TemporalOrder), linetype = 'solid') + geom_vline(data = subset(allScans, Drug == 'Methyl'), aes(xintercept = TemporalOrder), linetype = 'dashed')+
  labs(x = "Temporal Order", y = "TDProp1", title = "Timecourse for Each Subject") +
  theme_minimal()+geom_smooth()+
  facet_wrap(~Subjects, scales = "free_y")
...
new_labels <- paste0("sub-PSIL", seq_along(unique_values))
...
ggplot(allScansNoMeth, aes(x = Psilo, y = ResidualsReduced)) +
  ...
  labs(title = "Psilocybin vs. No-drug scans \n",
       x = "",
       y = "% Bottom-up") + scale_x_discrete(labels=c('No Drug','Psilocybin'))+
...
ggplot(drugScans, aes(x = Drug, y = ResidualsReduced)) +
  geom_jitter(width = 0.25, height = 0, alpha = 0.8,size=4,aes(color=People)) +  # Jittered points
  geom_boxplot(alpha = 0.2,outlier.shape = NA) +     # Boxplot
  labs(title = "Psilocybin vs. Active Placebo \n \n",
       x = "",
       y = "% Bottom-up") + scale_x_discrete(labels=c('Methylphenidate','Psilocybin'))+
  theme_minimal(base_size=26)+scale_color_manual(values = generated_colors)+theme(axis.text.x = element_text(size = 15))
...
ggplot(allScansNoMeth, aes(x = Condition2, y = ResidualsReduced)) +
  geom_jitter(width = 0.25, height = 0, alpha = 0.6,size=3) +  # Jittered points
  geom_boxplot(alpha = 0.2,outlier.shape = NA) +     # Boxplot
...
for (i in 1:num_bootstrap_samples){
  ...
  #cx_model <- lme(Complexity ~ FD + Drug+RemTRs, random = ~ 1 | Subjects, data = data)
...
```{r}
# calculate change in TD prop from No Drug to Drug to compare with MEQ positive emotion (inverse)
...
# replace MEQ ids with one-per-subject IDs
MEQ$Subjects[MEQ$Subjects=='PS93']='PS03'
MEQ$Subjects[MEQ$Subjects=='PS96']='PS16'
MEQ$Subjects[MEQ$Subjects=='PS98']='PS18'
MEQ$Subjects[MEQ$Subjects=='PS99']='PS19'

# Unique subjects
subjects <- unique(MEQ$Subjects)
# no sub 2
subjects = subjects[-c(7)]
# Initialize an empty list to store the merged data for each subject
merged_list <- list()
# initialize MEQ TD Prop change
MEQ$TDPropDelta=NULL
# omit methylphenidate from MEQ
MEQ=MEQ[MEQ$Class==3,]

# Loop through each subject
for (subject in subjects) {
  # Extract data for the current subject
  MEQ_sub <- subset(MEQ, Subjects == subject)
  scans_sub <- subset(allScansNoMeth, Subjects == subject)
  # no drug
  ND_scans_sub=scans_sub[scans_sub$Drug=='none',]
  P_scans_sub=scans_sub[scans_sub$Drug=='Psilo',]
  
  # Calculate the average TDProp1 for each TemporalOrder in scans_sub
  unique_sessions <- unique(scans_sub$TemporalOrder)
  session_avg_list <- list()
  
    # Within each session, average the scans
  for (session in unique_sessions) {
    # No drug condition
    NDscans_session <- subset(ND_scans_sub, TemporalOrder == session)
    avg_TDProp1_ND <- if (nrow(NDscans_session) > 0) mean(NDscans_session$TDProp1) else NA
    
    # Psilo condition
    Pscans_session <- subset(P_scans_sub, TemporalOrder == session)
    avg_TDProp1_P <- if (nrow(Pscans_session) > 0) mean(Pscans_session$TDProp1) else NA
    
    # Combine both averages into the session average list
    session_avg_list[[as.character(session)]] <- data.frame(
      Subjects = subject,
      TemporalOrder = session,
      Avg_TDProp1_ND = avg_TDProp1_ND,
      Avg_TDProp1_P = avg_TDProp1_P
    )
  }
  
  # Combine the averaged session data into a single data frame
  scans_avg <- do.call(rbind, session_avg_list)
  
  # get difference of TDProp1_P - average(Avg_TDProp1_ND)
  avg_ND_TDProp1 <- mean(scans_avg$Avg_TDProp1_ND, na.rm = TRUE)
  scans_avg$TDPropDelta <- scans_avg$Avg_TDProp1_P - avg_ND_TDProp1
  # only psil seshs
  scans_avg=scans_avg[!is.na(scans_avg$Avg_TDProp1_P),]
  # sort by temporal order
  scans_avg <- scans_avg[order(scans_avg$TemporalOrder), ]
  # Merge the calculated TDPropDelta back into MEQ for this subject
  for (i in 1:nrow(MEQ_sub)) {
    session <- MEQ_sub$session[i]
    delta <- scans_avg$TDPropDelta[i]
    if (length(delta) > 0) {
      MEQ$TDPropDelta[MEQ$Subjects == subject & MEQ$session == session] <- delta
    }
  }
}

# Check the dimensions of the merged data to ensure the merge was successful
...
ggplot(df_plot1, aes(x = Drug, y = Complexity, fill = Drug, group = SubjectTask, color = Subjects)) +
  ...
  geom_line(data = df_plot2, aes(group = SubjectTask, color = Subjects)) +
  scale_color_viridis_d(option = "plasma") +
  theme_minimal(base_size = 22)+xlab('MDMA')
