...
for task=["rs1" "rs2" "rs3" "rs4" "rs5" "rs6"]
		...
		for i=1:8
			...
			if exist(bvFP,'file')
				% check using props, but load in Dseg
                		bvFP=[commonFP subjList(s) '/Baseline' num2str(i) '/' subjList(s) '_Baseline' num2str(i) '_' task '_TemporalAutoCor_Spun_MOT.csv'];
                		bvFP=strjoin(bvFP,'');
                		bv=readmatrix(bvFP);	
			...
			if exist(pFP,'file')
				% check using props, but load in Dseg
                                pFP=[commonFP subjList(s) '/Between' num2str(i) '/' subjList(s) '_Between' num2str(i) '_' task '_TemporalAutoCor_Spun_MOT.csv'];
                                pFP=strjoin(pFP,'');
                                p=readmatrix(pFP);
			...
			if exist(m1FP,'file')
				% check using props, but load in Dseg
                                m1FP=[commonFP subjList(s) '/After' num2str(i) '/' subjList(s) '_After' num2str(i) '_' task '_TemporalAutoCor_Spun_MOT.csv'];
                                m1FP=strjoin(m1FP,'');
                                m1=readmatrix(m1FP);
			...
			if exist(m2FP,'file')
				% check using props, but load in Dseg
                                m2FP=[commonFP subjList(s) '/Drug' num2str(i) '/' subjList(s) '_Drug' num2str(i) '_' task '_TemporalAutoCor_Spun_MOT.csv'];
                                m2FP=strjoin(m2FP,'');
				m2=readmatrix(m2FP);
	...
	% save out matrix
	writematrix(outDF,strjoin(['/scratch/users/apines/data/' task '_Psil_MOTTemporalAutoCor_Merged_Spin_' num2str(n) '.csv'],''))
end
