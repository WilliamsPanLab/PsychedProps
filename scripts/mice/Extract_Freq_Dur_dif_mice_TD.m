...
for s=1:length(subjList);
	...
	for run=1:6
		...
		ofFP=strjoin([ofFP_base subjList(s) '_' num2str(run) '_TD_counts.mat'],'');
		ofFP2=strjoin([ofFP_base subjList(s) '_' num2str(run) '_TD_meanDur.mat'],'');
		% load in baseline csvs
		if exist(ofFP,'file')
			file=load(ofFP);
	        	outDF(s,(run+1))={file.TDcount};
			file2=load(ofFP2);
			outDF_dur(s,(run+1))={file2.meanTDdur};
		end
	% end for each run
	end
% end for each mouse
...
% save out matrix
writecell(outDF,'/oak/stanford/groups/leanew1/users/apines/data/mice_AngFreq_Merged_mice_LSD_TD.csv')
writecell(outDF_dur,'/oak/stanford/groups/leanew1/users/apines/data/mice_AngDur_Merged_mice_LSD_TD.csv')
end
