...
for s=1:length(subjList);
	...
	for run=1:6
		...
		ofFP=strjoin([ofFP subjList(s) '/' subjList(s) '_' num2str(run) '_avDMNSeg.csv'],'');
...
% save out matrix
writecell(outDF,'/oak/stanford/groups/leanew1/users/apines/data/mice_DMNSeg_Merged_mice_LSD.csv')
end
