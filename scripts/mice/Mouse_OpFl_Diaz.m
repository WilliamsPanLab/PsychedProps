function Mouse_OpFl_Diaz(subj,run)
...
basefp='/oak/stanford/groups/leanew1/users/apines/p50_mice/proc2/proc/drugs/processed_data/20200305/'
% load in specified scan
if run==1	
	fn = [basefp 'thy1gc6s_2mgkg_' subj '_preDiazepam2mgkg_1/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
...
% post 1
if run==2
	fn = [basefp 'thy1gc6s_2mgkg_' subj '_postDiazepam2mgkg_0/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
	else
...
% post 2
if run==3	
	fn = [basefp 'thy1gc6s_2mgkg_' subj '_postDiazepam2mgkg_5/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
	else    
...
% post 3
if run==4
	fn = [basefp 'thy1gc6s_2mgkg_' subj '_postDiazepam2mgkg_10/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
	else
...
% post 4
if run==5
	fn = [basefp 'thy1gc6s_2mgkg_' subj '_postDiazepam2mgkg_15/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
	else
