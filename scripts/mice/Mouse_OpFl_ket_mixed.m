function Mouse_OpFl_ket(subj,run)
...
% AP load in data: pre drug
basefp='/scratch/users/apines/p50_mice/proc/20180324/';
% load in specified scan
if run==1	
	pattern = g_ls([basefp 'thy1gc6s_*mgkg_' subj '_preKet_1/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']);
	fn=pattern{:};
	if exist(fn)
...
% post 1
if run==2
	pattern = g_ls([basefp 'thy1gc6s_*mgkg_' subj '_postKet_0/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']);
	fn=pattern{:};
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
...
% post 2
if run==3	
	pattern = g_ls([basefp 'thy1gc6s_*mgkg_' subj '_postKet_5/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']);
	fn=pattern{:};
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
...
% post 3
if run==4
	pattern = g_ls([basefp 'thy1gc6s_*mgkg_' subj '_postKet_10/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']);
	fn=pattern{:};
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
...
% post 4
if run==5
	pattern = g_ls([basefp 'thy1gc6s_*mgkg_' subj '_postKet_15/masked_dff_Gro_Masked_Sml_BP_Smoothed_Sml.h5']);
	fn=pattern{:};
	if exist(fn)
		data=h5read(fn, '/processed_data');
		formask=h5read(fn, '/mask');
...
outfp=[basefp subj '_vf_out_ket_' num2str(run) '.mat'];
save(outfp,'outputs');
% visualize
useAmplitude= true;
outFoldName=['/scratch/users/apines/mouseViz/' subj '_ket_' num2str(run)];
system(['mkdir ' outFoldName]);
vidName=[outFoldName '/vecField_ket'];
vidFps=15;
resizeScale=1;
vfScale=1;
