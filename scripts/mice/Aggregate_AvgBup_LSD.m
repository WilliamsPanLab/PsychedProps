...
for i=1:length(mList)
	...
	mousePreLSDfp=[parentfolder mouse '_1_Prop_TS_dmn.csv'];
...
for i=1:length(mList)
	...
	% for each session
		...
		sesh=seshs(s-1);
		% load in mouse tp1 (LSD)
        	mousePostLSDfp=[parentfolder mouse '_' num2str(sesh) '_Prop_TS_dmn.csv'];
...
for i=1:length(mDexList)
        ...
        mousePreDexfp=[parentfolder 'Dex/' mouse '_1_Prop_TS_dmn.csv'];
...
for i=1:length(mDiazList)
        ...
        mousePreDiazfp=[parentfolder 'Diaz/' mouse '_1_Prop_TS_dmn.csv'];
...
hold on;
contour(Mask, [0.5 0.5], 'k', 'LineWidth', 1.5); % Add a black contour line
print('~/LSD_NoDrugMinPost.png','-dpng')
