...
for s=1:length(subjList);
	...
	for run=1:6
		...
		ofFP=strjoin([ofFP subjList(s) '/' subjList(s) '_' num2str(run) '_avDMNWin.csv'],'');
...
% save out matrix
writecell(outDF,'/oak/stanford/groups/leanew1/users/apines/data/mice_DMNWin_Merged_mice_LSD.csv')
end
