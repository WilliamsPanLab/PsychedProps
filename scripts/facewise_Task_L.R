...
if (is.na(iteration) || iteration < 1 || iteration > 6) {
	  stop("This script is intended for iterating over the faces in chunks: 1, 2, 3, 4, 5, or 6. Tell me which chunk you want or find yourself another script.")
}

# Calculate the range of iterations based on the input
start_iteration <- (iteration - 1) * 300 + 1
end_iteration <- iteration * 300
...
for (f in start_iteration:end_iteration){
	...
	# extract t stat
	Tg_L[f]=summary(mmodel)$tTable[3,4]
	pg_L[f]=summary(mmodel)$tTable[3,5]
	Tw_L[f]=summary(mmodel)$tTable[4,4]
        pw_L[f]=summary(mmodel)$tTable[4,5]	
}
# write out t stats
write.csv(Tg_L,paste0('/scratch/users/apines/DMN/DMN_Prop_Tstats_L_Taskg_',as.character(iteration),'.csv'))
write.csv(pg_L,paste0('/scratch/users/apines/DMN/DMN_Prop_ps_L_Taskg_',as.character(iteration),'.csv'))
write.csv(Tw_L,paste0('/scratch/users/apines/DMN/DMN_Prop_Tstats_L_Taskw_',as.character(iteration),'.csv'))
write.csv(pw_L,paste0('/scratch/users/apines/DMN/DMN_Prop_ps_L_Taskw_',as.character(iteration),'.csv'))
