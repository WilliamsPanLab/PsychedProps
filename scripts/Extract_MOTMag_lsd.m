function Extract_DMNMag_lsd(subj,sesh,task)
% define some paths 
...
fp=[childfp '/' subj '_' sesh '_' task '_OpFl.mat'];
...
mwAndTSNR_L=gifti(mwAndTSNR_L).cdata(:,3);
mwAndTSNR_R=gifti(mwAndTSNR_R).cdata(:,3);
mw_L=zeros(1,2562);
...
%% k = 1 to select DMN.
Dnet_LH=networks.nets.Lnets(:,3);
Dnet_RH=networks.nets.Rnets(:,3);
...
writetable(T,[outFP '/' subj '_' sesh '_' task '_MOTMag.csv'],'WriteRowNames',true)
% save out facewise
writetable(leftAvgMag,[outFP '/' subj '_' sesh '_' task '_MOTMags_L.csv'],'WriteRowNames',true)
writetable(rightAvgMag,[outFP '/' subj '_' sesh '_' task '_MOTMags_R.csv'],'WriteRowNames',true)
